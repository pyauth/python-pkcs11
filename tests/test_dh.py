...
def test_derive_key(session: pkcs11.Session) -> None:
    ...
    parameters = session.create_domain_parameters(
        KeyType.DH, {Attribute.PRIME: prime, Attribute.BASE: [0x2]}, local=True
    )
    ...
    assert alice_session[Attribute.VALUE] == bob_session[Attribute.VALUE]
...
def test_load_params(session: pkcs11.Session) -> None:
    # This is RFC5114 #2
...
def test_generate_params(session: pkcs11.Session) -> None:
    ...
    assert isinstance(params, DomainParameters)
    assert params[Attribute.PRIME_BITS] == 512
    assert len(params[Attribute.PRIME]) * 8 == 512
